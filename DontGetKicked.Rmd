---
title: "DontGetKicked"
output: html_document
---

Data URL:  https://www.kaggle.com/c/DontGetKicked/data


```{r}
rm(list = ls())
library(tidyverse)
library(ggplot2)
```

```{r}
train <- read.csv(file = "data/training.csv", header = TRUE, sep = ",")
glimpse(train)
```

<<<<<<< HEAD
First, let's start cleaning the data.
Some variables are not very relevent at first look: 

=======

```{r}
#Change purchase date to purchase year (or maybe we can just delete this variable)
class(train$PurchDate)
train$PurchDate <- as.Date(train$PurchDate, format = "%m/%d/%Y")
train$PurchDate <- format(train$PurchDate, "%Y")
train$PurchDate <- as.integer(train$PurchDate)
class(train$PurchDate)
head(train)
colnames(train)[3] <- "PurchYear"
```


```{r}
#clean transmission data
levels(train$Transmission)
train <- train%>%
  mutate(Transmission = replace(Transmission, Transmission=="Manual", "MANUAL"))
  
train <- train%>%
  filter(Transmission =="MANUAL"| Transmission =="AUTO")

train$Transmission <- factor(train$Transmission)
```


```{r}
#check wheeltype (Delete NULL?)
levels(train$WheelTypeID)
table(train$WheelTypeID)

```

```{r}

#clean size
#check size

levels(train$Size)
table(train$Size)
train<- train%>%
  filter(Size!="NULL")
train$Size <- factor(train$Size)
```


```{r}

#Clean Nationality

#Nationality

levels(train$Nationality)
table(train$Nationality)
train$Nationality <- factor(train$Nationality)
```

```{r}
levels(train$TopThreeAmericanName)
table(train$TopThreeAmericanName)
train$TopThreeAmericanName <- factor(train$TopThreeAmericanName)
```

```{r}
print(c("Number of NULLs in Model: ",nrow(train%>%
  filter(Model=="NULL"))))
print(c("Number of NULLs in SubModel: ",nrow(train%>%
  filter(SubModel=="NULL"))))
print(c("Number of NULLs in Trim: ",nrow(train%>%
  filter(Trim=="NULL"))))
print(c("Number of empty strings in Model: ",nrow(train%>%
  filter(Model==""))))
print(c("Number of empty strings in SubModel: ",nrow(train%>%
  filter(SubModel==""))))
print(c("Number of empty strings in Trim: ",nrow(train%>%
  filter(Trim==""))))
print(c("Number of NULLs in Size: ",nrow(train%>%
  filter(Size=="NULL"))))

```
<<<<<<< HEAD

#split the data set
```{r}
set.seed(66)
index <- sample(72969, 20000)
training <- train[-index,]
testing <- train[index,]
```


#Try SVM
```{r}
library(e1071)
svm_tune <- tune(svm,                            # find a best set of parameters for the svm model           
                 IsBadBuy~VehicleAge+VehBCost+VehOdo+WarrantyCost,         
                 data = training,
                 kernel="linear", 
                 ranges = list(cost = 10^(-5:0)))
```


```{r}
print(svm_tune)
best_svm_mod <- svm_tune$best.model
=======
```{r}
train_modified <- train[,-c(8,9,10,14,27,28,29)]

```
```{r}
install.packages("dummies")
library(dummies)
>>>>>>> 1ce666fc0efb2ea13ee85d6c3bf4b48f2123a7f7
```


```{r}
<<<<<<< HEAD
testing$svm_pred_class <- predict(best_svm_mod, testing) 
testing$svm_dv<-as.numeric(attr(predict(best_svm_mod, testing, decision.values = TRUE),"decision.values"))

table(testing$IsBadBuy==test$logit_pred_class)
```


```{r}
library(pROC)
svm_roc<-roc(testing$IsBadBuy,testing$svm_dv,auc=TRUE)
plot(svm_roc,print.auc=TRUE, col="red")
```

=======
#dummy(train_modified$, sep = "_")
which(train_modified == "NULL", arr.ind = TRUE)
train_modified <- train_modified[!(train_modified$MMRCurrentAuctionAveragePrice=="NULL" & train_modified$MMRCurrentAuctionCleanPrice=="NULL"),]

sum(train_modified == "NULL")
```
```{r}
#train_modified <- train_modified[,-1]
write.csv(train_modified, file = "train_modified.csv",row.names=FALSE)
```
>>>>>>> 1ce666fc0efb2ea13ee85d6c3bf4b48f2123a7f7
